...
!!                                      !fcst legth, 00-24hr, 24-48hr, 48-72hr for 12Z cycle
      ...
      integer, allocatable :: missday(:,:,:)                                 !used to mark days with missing values for each model
      integer, allocatable :: miss(:,:)                                      !used to mark days with missing values for uniform sampling
...
!--------------------------------------------------------
      ...
      allocate ( missday(nfhr,nday,nexp) )
      allocate ( miss(nfhr,nday) )
...
!--read data
      ...
      do 20 id = 1, nday                    
       ...
       do 30 mh = 1, nfhr     !fcst hour
         rewind 11
         line2skip=(mh-1)*11*4+nreg*4
         do lskip = 1, line2skip
           read (11,900,end=30)   
         enddo
         read (11,902,end=30) mfhour                         
         ih=((mfhour+cycle)-12)/24
         if (ih .gt. nfhr) goto 30
         ihour=ih*24+12-cycle
         do line=1,3
          nn=3*(line-1)
          read (11,901,end=30) obs(nn+1,ih,id,np),fcs(nn+1,ih,id,np), hit(nn+1,ih,id,np),tot(nn+1,ih,id,np), &
 ...
 10   continue
      ...
      do 13 id = 1, nday                    
      do 13 np = 1,nexp
       if ( missday(ih,id,np) .eq. 1) miss(ih,id)=1
  13  continue
...
!--compute skill scores
      ...
      do 50 ic = 1, ncat
         do 60 id = 1, nday
          if (tot(ic,ih,id,np).eq.-9999.0 .or. miss(ih,id).eq.1 ) then
