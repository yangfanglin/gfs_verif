...
#^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 if [ $myhost = $myclient -o $machine != WCOSS ]; then
  if [ -s $indir/$oua ]; then
   ...
   if [ -s $indir/$ina ]; then $APRUN $s2g $indir/$ina $oua tmpa.ctl $idrt $lonb $latb ;fi 
  fi
  if [ -s $indir/$oug ]; then
   ln -fs $indir/$oug .
  else
   if [ -s $indir/$ing ]; then $APRUN $s2g $indir/$ing $oug tmpg.ctl $idrt $lonb $latb ;fi 
  fi
 elif [ $machine = WCOSS ]; then
  scp -pB ${LOGNAME}@${CLIENT}:$indir/$oua $oua          
  if [ $? -ne 0 ]; then 
   scp -pB ${LOGNAME}@${CLIENT}:$indir/$ina $ina          
   if [ $? -eq 0 ]; then $APRUN $s2g $ina $oua tmpa.ctl $idrt $lonb $latb ;fi 
  fi
  scp -pB ${LOGNAME}@${CLIENT}:$indir/$oug $oug          
  if [ $? -ne 0 ]; then 
   scp -pB ${LOGNAME}@${CLIENT}:$indir/$ing $ing          
   if [ $? -eq 0 ]; then $APRUN $s2g $ing $oug tmpg.ctl $idrt $lonb $latb ;fi 
  fi
 fi
...
EOF1
   ...
   if [ $batch = NO ]; then 
    ...
    $SUBJOB -a $ACCOUNT -q $CUE2FTP -g $GROUP -p 1/1/S -t 0:30:00  -r 256/1 -j hpssget$$ -o hpssget$$.out $datadir/hpssget$$.sh 
   fi
...
EOF2
  ...
  sleep 30
  if [ -s $ina ]; then $APRUN $s2g $ina $oua tmpa.ctl $idrt $lonb $latb ;fi 
  if [ -s $ing ]; then $APRUN $s2g $ing $oug tmpg.ctl $idrt $lonb $latb ;fi 
  cp -p $oua $expdir/$exp/.
  cp -p $oug $expdir/$exp/.
 #-----------------------------------------------------------
 fi
...
cat >$ctldir/${exp}_${dump}anl.ctl <<EOF
...
cat >$ctldir/${exp}_${dump}ges.ctl <<EOF
