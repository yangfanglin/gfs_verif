...
export GADDIR=${GADDIR:-/scratch4/NCEPDEV/global/save/Mallory.Row/grads/}
#------------------------------------------------------------------
#------------------------------------------------------------------
...
export GADDIR=${GADDIR:-/usrx/local/dev/GrADS/data}
...
if [ $mapobs = "yes"  ]; then
#=====================================================================
climo_ceres=${climo_ceres:-no} ;#plot CERES climatology (yes) or monthly means (no); "no" valid Mar2000-Jun2016
use_calipso_cldfrc=${use_calipso_cldfrc:-no} ;#plot cloud fractions use CALIPSO climatology; **only use if climo_ceres set to 'yes'**
climo_gpcp=${climo_gpcp:-no}   ;#plot GPCP climatology (yes) or monthly means (no); "no" valid Mar2000-Jul2016
climo_ghcncams=${climo_ghcncams:-no} ;#plot GHCN CAMS climatology (yes) or monthly means (no); "no" valid Mar2000-Nov2016
cloud=${cloud:-yes}           ;#plot cloud distributions
longw=${longw:-yes}           ;#plot longwave flux
solar=${solar:-yes}           ;#plot solar flux
preci=${preci:-yes}           ;#plot precipitation
pwat=${pwat:-yes}             ;#plot column-integrated water vapor
t2m=${t2m:-yes}               ;#plot T2m
clwp=${clwp:-yes}             ;#plot column-integrated cloud liquid and ice water      


export climo_ceres=$climo_ceres
export use_calipso_cldfrc=$use_calipso_cldfrc
export climo_gpcp=$climo_gpcp
export climo_gchncams=$climo_ghcncams
...
if [ $climo_ceres = "yes" ]; then
   if [ $cmon2 -lt $cmon ]; then cmon2=$((cmon2+12)); fi
   if [ $grid = "G2" ]; then
      echo "Using 2.5x2.5 CERES Climo"
      export ob_ctl=${obdata}/ceres/CERES_climo_25.ctl
      export ob_name="CERES Climo 00-16"
   elif [ $grid = "G3" ]; then
      echo "Using 1.0x1.0 CERES Climo"
      export ob_ctl=${obdata}/ceres/CERES_climo_1.ctl
      export ob_name="CERES Climo 00-16"
   elif [ $grid = "G4" ]; then
      echo "No 0.5x0.5 CERES Climo"
      exit
   else
      echo "Invalid Grid Option"
      exit
   fi
elif [ $climo_ceres = "no" ]; then
   export yeardiff=$(($year - 2000))
   export yeardiffmon=$((yeardiff * 12))
   cmon=$((yeardiffmon + cmon -2))
   export yeardiff2=$(($year2 - 2000))
   export yeardiffmon2=$((yeardiff2 * 12))
   cmon2=$((yeardiffmon2 + cmon2 -2))
   if [ $grid = "G2" ]; then
      echo "Using 2.5x2.5 CERES Monthly Means"
      export ob_ctl=${obdata}/ceres/CERES_monthlymeans_25.ctl
      export ob_name="CERES Mon. Mean" 
   elif [ $grid = "G3" ]; then
      echo "Using 1.0x1.0 CERES Monthly Means"
      export ob_ctl=${obdata}/ceres/CERES_monthlymeans_1.ctl
      export ob_name="CERES Mon. Mean"
   elif [ $grid = "G4" ]; then
      echo "No 0.5x0.5 CERES Monthly Means"
      exit
   else
      echo "Invalid Grid Option"
      exit
   ...
   echo "Invalid Climo Option"
   exit
fi
###############################################
if [ $use_calipso_cldfrc = "yes" ]; then
   if [ $grid = "G2" ]; then
      echo "Using 2.5x2.5 CALIPSO Climo"
      export ob_ctl_calipso=${obdata}/calipso/CALIPSO_climo_25.ctl
      export ob_name_calipso="CALIPSO Climo 06-10"
   elif [ $grid = "G3" ]; then
      echo "Using 1.0x1.0 CALIPSO Climo"
      export ob_ctl_calipso=${obdata}/calipso/CALIPSO_climo_1.ctl
      export ob_name_calipso="CALIPSO Climo 06-10"
   elif [ $grid = "G4" ]; then
      echo "No 0.5x0.5 CERES Climo"
      exit
...
#######################################
if [ $climo_gpcp = "yes" ]; then
   if [ $pmon2 -lt $pmon ]; then pmon2=$((pmon2+12)); fi
   if [ $grid = "G2" ]; then
      echo "Using 2.5x2.5 GPCP Climo"
      export ob_ctl_precip=${obdata}/gpcp/GPCP_climo_25.ctl
      export ob_name_precip="GPCP Climo 79-16"
   elif [ $grid = "G3" ]; then
      echo "Using 1.0x1.0 GPCP Climo"
      export ob_ctl_precip=${obdata}/gpcp/GPCP_climo_1.ctl
      export ob_name_precip="GPCP Climo 79-16"
   elif [ $grid = "G4" ]; then
      echo "No 0.5x0.5 GPCP Climo"
      exit
   else
      echo "Invalid Grid Option"
      exit
   fi
elif [ $climo_gpcp = "no" ]; then
   export yeardiff=$(($year - 2000))
   export yeardiffmon=$((yeardiff * 12))
   pmon=$((yeardiffmon + pmon -2))
   export yeardiff2=$(($year2 - 2000))
   export yeardiffmon2=$((yeardiff2 * 12))
   pmon2=$((yeardiffmon2 + pmon2 -2))
   if [ $grid = "G2" ]; then
      echo "Using 2.5x2.5 GPCP Monthly Means"
      export ob_ctl_precip=${obdata}/gpcp/GPCP_monthlymeans_25.ctl
      export ob_name_precip="GPCP Mon. Mean"
   elif [ $grid = "G3" ]; then
      echo "Using 1.0x1.0 GPCP Monthly Means"
      export ob_ctl_precip=${obdata}/gpcp/GPCP_monthlymeans_1.ctl
      export ob_name_precip="GPCP Mon. Mean"
   elif [ $grid = "G4" ]; then
      echo "No 0.5x0.5 GPCP Monthly Means"
      exit
   else
      echo "Invalid Grid Option"
      exit
...
#############################################
if [ $climo_ghcncams = "yes" ]; then
   if [ $tmon2 -lt $tmon ]; then tmon2=$((tmon2+12)); fi
   if [ $grid = "G2" ]; then
      echo "Using 2.5x2.5 GHCN CAMS Climo"
      export ob_ctl_t2m=${obdata}/ghcn_cams/GHCN_CAMS_climo_25.ctl
      export ob_name_t2m="GHCN CAMS Climo 48-16"
   elif [ $grid = "G3" ]; then
      echo "Using 1.0x1.0 GHCN CAMS Climo"
      export ob_ctl_t2m=${obdata}/ghcn_cams/GHCN_CAMS_climo_1.ctl
      export ob_name_t2m="GHCN CAMS Climo 48-16"
   elif [ $grid = "G4" ]; then
      echo "No 0.5x0.5 GHCN CAMS Climo"
      exit
   else
      echo "Invalid Grid Option"
      exit
   fi
elif [ $climo_ghcncams = "no" ]; then
   export yeardiff=$(($year - 2000))
   export yeardiffmon=$((yeardiff * 12))
   tmon=$((yeardiffmon + tmon -2))
   export yeardiff2=$(($year2 - 2000))
   export yeardiffmon2=$((yeardiff2 * 12))
   tmon2=$((yeardiffmon2 + tmon2 -2))
   if [ $grid = "G2" ]; then
      echo "Using 2.5x2.5 GHCN CAM Monthly Means"
      export ob_ctl_t2m=${obdata}/ghcn_cams/GHCN_CAMS_monthlymeans_25.ctl
      export ob_name_t2m="GHCN CAMS Mon. Means"
   elif [ $grid = "G3" ]; then
      echo "Using 1.0x1.0 GHCN CAMS Monthly Means"
      export ob_ctl_t2m=${obdata}/ghcn_cams/GHCN_CAMS_monthlymeans_1.ctl
      export ob_name_t2m="GHCN CAMS Mon. Means"
   elif [ $grid = "G4" ]; then
      echo "No GHCN CAMS 0.5x0.5 Monthly Means"
      exit
   else
      echo "Invalid Grid Option"
      exit
...
'reinit'; 'set font 1'
'set display color  white'
if  ($use_calipso_cldfrc = yes)
  'xdfopen $ob_ctl_calipso'
  mdc.1="${ob_name_calipso}"
else
  'xdfopen $ob_ctl'
  mdc.1="${ob_name}"
endif
...
*-----
  ...
  'define sn1=ave(${varob},t=${cmon},t=${cmon2})'
...
#      ob_ctl=${obdata}/ceres/CERES_climo_25.ctl
      varob=LW_sfc_up
      varname="Sfc Up LW"
  fi
...
*-----
  ...
  'define sn1=ave(LW_sfc_down.1+LW_toa_up.2-LW_sfc_up.3,t=${cmon},t=${cmon2})'
  'define yn1=aave(sn1,lon=$lon1,lon=$lon2,lat=$lat1,lat=$lat2)'
...
*-----
  ...
  'define utoa1=ave(SW_toa_up.1, t=$cmon, t=$cmon2)'
  'define usfc1=ave(SW_sfc_up.2, t=$cmon, t=$cmon2)'
  'define dsfc1=ave(SW_sfc_down.3, t=$cmon, t=$cmon2)'
  'define dtoa1=ave(SW_toa_down.4, t=$cmon, t=$cmon2)'
...
'reinit'; 'set font 1'
'set display color  white'
  'xdfopen $ob_ctl_precip'   
   mdc.1="${ob_name_precip}"
  'open $ctl1_f1'
  'open $ctl1_f2'
  'open $ctl1_f3'
  'open $ctl1_f4'
...
*-----
  ...
  'define sn1=ave(${varob}, t=${pmon},t=${pmon2})'
  'define yn1=aave(sn1,lon=$lon1,lon=$lon2,lat=$lat1,lat=$lat2)'
...
* water vapor only
  'define sn1=ave(${scal}*${varob}, t=${cmon}, t=${cmon2})'
  'define yn1=aave(sn1,lon=$lon1,lon=$lon2,lat=$lat1,lat=$lat2)'
...
export ob_ctl=${ob_ctl}
#export ob_name="CERES"
export nmd=`expr $nexp + 1 `

for var in CWATclm ;  do
      varob=clwp
      varob2=ciwp
      varname="Column Water and Ice (g/m^2)"
      scal=1000
...
*-----
  ...
  'define sn1=ave(${varob}+${varob2}, t=${cmon}, t=${cmon2})'
  'define yn1=aave(sn1,lon=$lon1,lon=$lon2,lat=$lat1,lat=$lat2)'
...
while ( n <= ${nexp} )
  ...
  f4=(n-1)*4+5
  say f1 f2 f3 f4 
  'define sn'%nn'=maskout(${scal}*ave((${var}.'%f1' + ${var}.'%f2' + ${var}.'%f3' + ${var}.'%f4')/4, time=${sdate},time=${edate}),sn1)-sn1'
  'define yn'%nn'=aave(sn'%nn',lon=$lon1,lon=$lon2,lat=$lat1,lat=$lat2)'
  n=n+1
endwhile
...
*   'set clab forced'
    ...
    'set strsiz 0.12 0.12'
    if(i=1); 'draw string 'titlx' 'titly ' 'mdc.1'  'a; endif
*    if(i=1); 'draw string 'titlx' 'titly ' 'mdc.1' (CLWP+CIWP)  'a; endif
    if(i>1); 'draw string 'titlx' 'titly ' 'mdc.i' - 'mdc.1'  'a; endif
...
*-----
  ...
  'define sn1=ave(${varob}-273.15, t=${tmon}, t=${tmon2})'
  'define yn1=aave(sn1,lon=$lon1,lon=$lon2,lat=$lat1,lat=$lat2)'
