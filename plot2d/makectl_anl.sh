...
for exp in $expnlist; do
 if [ $exp = fim ] ; then 
  nlev=26
  levlist="1000 975 950 925 900 850 800 750 700 650 600 550 500 450 400 350 300 250 200 150 100 75 50 25 20 10"
 fi 
done
for exp in $expnlist; do
 if [ $exp = ecm ] ; then 
...
for hr in ${fhname[3]}; do
 ...
 rm -f ${datadir}/xtmp
  ...
  if [ $exp = "gfs" ]; then
    ...
    filetmp=${expdir}/${exp}/pgbanl$dump$adate
  fi
...
EOF
#-----------------------------------------------
gribmap -0 -i $ctldir/${exp}_${hr}_anl.ctl
