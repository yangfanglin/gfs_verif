...
#  Author: Fanglin.Yang@noaa.gov, 301-6833722, April 2007 
#---------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------
...
export GADDIR=${GADDIR:-/usrx/local/grads/data}
...
*---------------------------------------------------------
   ...
   while (i <= $nmd)
    ...
    ln=sublin(result,11); wd=subwrd(ln,2); a=substr(wd,1,4)
    if ( a>0 )
      'set strsiz 0.13 0.13'; yt=yt-yy
      'set line 'cco.i' 'cst.i' 11'; 'draw line 'xt' 'yt' 'xt1' 'yt
...
topdir=${rundir}/${vtype}
set -A give $vnamlist none none none none
nva=`echo $vnamlist | wc -w`

merge=F
if [ $nva -eq 1 -a ${give[0]} = "HGT_WV1/10-20" ]; then merge=T; fi             
if [ $nva -eq 4 -a ${give[3]} = "HGT_WV1/10-20" ]; then merge=T; fi             
...
#----------------------
 ...
 reg1=`echo $reg | sed "s?/??g"`
 tag=${lev}_${reg1}
 f1=$topdir/HGTWV10-20/cordieoff_HGTWV10-20_${tag}.png
 f2=$topdir/HGTWV4-9/cordieoff_HGTWV4-9_${tag}.png
 f3=$topdir/HGTWV0-3/cordieoff_HGTWV0-3_${tag}.png
 f4=$topdir/HGT/cordieoff_HGT_${tag}.png

 nsleep=0; tsleep=120;  msleep=60
 while [ $nsleep -lt $msleep ]; do 
  if [ ! -s $f1 -o ! -s $f2 -o ! -s $f3 -o ! -s $f4 ]; then 
   sleep $tsleep
  else
   nsleep=$msleep
  fi
 ...
 $imgconvert $f1 $f2  -rotate 90 -append x1.png
 $imgconvert $f3 $f4  -rotate 90 -append x2.png
