...
export runhpss=${runhpss:-YES}                                         ;#run hpsstar in batch mode to get missing data 
...
if [ $batch != YES ]; then APRUN=""; fi
export APRUN=${APRUN:-""}

NDAT=${NDAT:-1}     ;#number of days to verify for each batch job
if [ $DATEST -eq $DATEND ]; then NDAT=0; fi
...
if [ $DAYBEG -le $DATEND ]; then
 ...
 $SUBJOB -e batch,DAYBEG,DATEND,listvar,$listvar -a $ACCOUNT -q $CUE2RUN -g $GROUP -p 1/1/$share  -r $memory/1 -t 6:00:00  -j $jobname -o $jobout $0   
fi
