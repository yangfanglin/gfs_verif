...
rm $rundir/g2o*.out
$SUBJOB -a $ACCOUNT  -q transfer -g $GROUP -p 1/1/S -r 512/1 -t 3:00:00 -j getgfsbufr \
        -o $rundir/get_opsgfs_prepbufr.out $vsdbhome/grid2obs/get_opsgfs_prepbufr.sh
#$SUBJOB -a $ACCOUNT  -q transfer -g $GROUP -p 1/1/S -r 512/1 -t 3:00:00 -j getndasbufr \
#        -o $rundir/get_ndas_prepbufr.out $vsdbhome/grid2obs/get_ndas_prepbufr.sh   
$SUBJOB -a $ACCOUNT  -q transfer -g $GROUP -p 1/1/S -r 512/1 -t 3:00:00 -j getnambufr \
...
##-parallel GFS
export cyclist="00 06 12 18"                    ;#forecast cycles
export expnlist="pr4devb"                           ;#experiment names
export expdlist="/global/noscrub/emc.glopara/archive" 
export dumplist=".gfs."  
export complist="$(hostname)"
export fhoutair="6"                         ;#forecast output frequency in hours for raobs vrfy
export fhoutsfc="3"                         ;#forecast output frequency in hours for sfc vrfy
...
JJOB=${vsdbhome}/grid2obs/grid2obs.sh
#$SUBJOB -e listvar,$listvar -a $ACCOUNT  -q $CUE2RUN -g $GROUP -p 1/1/$share -r $memory/1 \
#        -t 6:00:00 -j g2opara -o $rundir/g2opara.out $JJOB 
