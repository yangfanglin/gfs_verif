...
export mapdir=${mapdir:-$tmpdir/maps/g2o}
...
for cyc  in $cyclist ; do
   export exedir=${tmpdir}/${cyc}Z/${regname}/${vnam}
...
set -A mdname $mdlist
set -A mdnamec `echo $caplist |tr "[a-z]" "[A-Z]" `
...
*--find maximum and minmum values
   ...
   while (i <= $nmd)
    ...
    if ( count>0 )
      ...
      'set cstyle 'cst.i; 'set cthick 10'; 'set cmark 'cma.i; 'set ccolor 'cco.i
...
*-----------------------------------------
*--plot difference between fcst and obs
*-----------------------------------------
  ymin=ymin-4.05; ymax=ymin+4
  ywd=4.0; yy=ywd/16

  xt=xmin+0.2; xt1=xt+0.3; xt2=xt1+0.1; yt=ymax-0.01*ywd ;*for legend  
  zxt=xt2+3.5; zxt1=zxt+0.3; zxt2=zxt1+0.1; zyt=ymax-0.01*ywd
  titlx=xmin+0.5*xwd;  titly=ymax+0.20                  ;*for tytle
  xlabx=xmin+0.45*xwd;  xlaby=ymin-0.60
  'set parea 'xmin' 'xmax' 'ymin' 'ymax
   ...
   while (i <= $nmd)
    ...
    if ( count>0 )
      if( i <=4); 
        'set strsiz 0.13 0.13'; yt=yt-yy
        'set line 'cco.i' 'cst.i' 11'; 'draw line 'xt' 'yt' 'xt1' 'yt
*       'set string 'cco.i' bl 6';     'draw string 'xt2' 'yt' 'mdc.i'-Obs 'mean'  'count
        'set string 'cco.i' bl 6';     'draw string 'xt2' 'yt' 'mdc.i'-Obs 'mean
      else
        'set strsiz 0.13 0.13'; zyt=zyt-yy
        'set line 'cco.i' 'cst.i' 11'; 'draw line 'zxt' 'zyt' 'zxt1' 'zyt
*       'set string 'cco.i' bl 6';     'draw string 'zxt2' 'zyt' 'mdc.i'-Obs 'mean'  'count
        'set string 'cco.i' bl 6';     'draw string 'zxt2' 'zyt' 'mdc.i'-Obs 'mean
      endif
...
#-------------------------------
  if [ $copymap = "YES" ]; then
    mkdir -p $mapdir/g2o_${cyc}Z/sfc
    cp fo*.png  $mapdir/g2o_${cyc}Z/sfc/.
    cp rms*.png  $mapdir/g2o_${cyc}Z/sfc/.
  fi
#------------------------------
...
EOF
 ...
 if [ $? -ne 0 ]; then
  $SUBJOB -a $ACCOUNT -q $CUE2FTP -g $GROUP -p 1/1/S -t 0:30:00 -r 64/1 $exedir/ftpcard.sh
 fi
fi
