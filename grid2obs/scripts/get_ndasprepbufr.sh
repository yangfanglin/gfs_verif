...
#  valid on and before 03/19/2017
...
export COMROT=${COMROTNAM:-/com}
export RUNDIR=${RUNDIR:-/stmpd2/$LOGNAME/g2o}
export ndasbufr_arch=${ndasbufr_arch:-/global/noscrub/Fanglin.Yang/prepbufr/ndas}
...
if [ -s $COMROT/nam/prod/ndas.${date1}/$ndas1 ]; then
  cp $COMROT/nam/prod/ndas.${date1}/$ndas1 .
elif [ -s $ndasbufr_arch/ndas.${date1}/$ndas1 ]; then
  cp $ndasbufr_arch/ndas.${date1}/$ndas1 .
...
if [ -s $ndas1 ]; then
  ...
  elif [ -s $ndasbufr_arch/ndas.${date2}/$ndas2 ]; then
    cp $ndasbufr_arch/ndas.${date2}/$ndas2 .
  elif [ $runhpss = YES ]; then
  ...
  if [ -s $ndas2 ]; then
    ...
    elif [ -s $ndasbufr_arch/ndas.${date3}/$ndas3 ]; then
      cp $ndasbufr_arch/ndas.${date3}/$ndas3 .
    elif [ $runhpss = YES ]; then
